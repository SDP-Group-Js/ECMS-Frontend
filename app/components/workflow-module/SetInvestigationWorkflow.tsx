...
const SetInvestigationWorkflow = ({
  ...
  const createWorkflow = () => {
    const body = {
      workflow: stages,
      stages: [
        ...stages.map((stage, index) => ({ name: stage, order: index + 1 })),
      ],
    };
  ...
  return (
    <Modal onClick={closeForm}>
      ...
      >
        ...
        <div >
        {selectedWorkflow && selectedWorkflow.stages.map((stage: { name: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined; }, index: number) => (
