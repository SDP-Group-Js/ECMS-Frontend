...
const InvestigationStage = ({
  ...
  return (
    <div className="m-auto mt-5 w-5/6 rounded-lg p-2 border-2">
        ...
        <>
          ...
          <div>
            <div className="flex justify-between items-center">
              ...
              {active && (
                <button onClick={() => setOpenTakeAction(true)} className="bg-gray-600 text-white font-bold text-lg w-fit px-2 py-1 rounded-lg">
                  Take Action
                </button>
              )}
            </div>
      ...
      {selectedAction && (
        <ViewActionDetail closeModal={() => setSelectedAction(null)} action={selectedAction} />
      )}
    </div>
  );
};
...
const TakeActionForm = ({closeModal}: any) => {
  return (
    <Modal onClick={closeModal}>
      <div onClick={e => e.stopPropagation()} className="bg-white relative rounded-lg p-5 w-1/3">
        <button onClick={closeModal} className="text-lg text-white bg-red-700 w-fit h-fit p-1 absolute top-2 right-2 rounded-full">
          <FaTimes />
        </button>
        ...
        <button className="px-2 py-1 font-bold rounded text-lg text-white bg-red-700">Take Action</button>
      </div>
    </Modal>
  )
}
...
const ViewActionDetail = ({action, closeModal}: any) => {
  return (
    <Modal onClick={closeModal}>
      <div onClick={e => e.stopPropagation()} className="relative bg-white rounded-lg w-5/6 p-3">
        <button onClick={closeModal} className="bg-red-700 p-1 rounded-full text-xl text-white absolute top-3 right-3">
          <FaTimes />
        </button>
        <div className="flex gap-2 items-center">
